...
# 목적: 5분 내로 주요 엔드포인트의 응답시간(p50/p95/p99/평균/최대)을 CSV로 남기고,
#       콘솔에 요약을 출력합니다. 이후 perf-gate(퇴행 판정)에서 이 CSV를 사용합니다.
#
# 사용 예시:
...
param(
    ...
    [int]$Iterations = 200,
    [int]$Concurrency = 1, # 아직 단일 스레드만 지원(확장 여지용 옵션)
...
function Measure-Endpoint {
    ...
    for ($i=1; $i -le $Iterations; $i++) {
        ...
        $sw.Stop()
        if ($i -le 1) { Write-Host "[DEBUG] ${name} ${i}회차 측정 완료 (status=$status)" }
    ...
    Write-Host "[DEBUG] 함수 내부 rows 개수: $($rows.Count)"
...
Write-Host "[DEBUG] echoRes 타입: $($echoRes.GetType().FullName)"
Write-Host "[DEBUG] echoRes.rows 개수: $($echoRes.rows.Count)"
...
Write-Host "[DEBUG] allRows 누적 개수: $($allRows.Count)"

# CSV 저장
# Count가 일부 환경에서 비어 보이는 문제가 있어 Measure-Object로 보강
$rowCount = ($allRows | Measure-Object).Count
Write-Host "[DEBUG] 수집된 행 타입: $($allRows.GetType().FullName)"
Write-Host "[DEBUG] 수집된 행 수: $rowCount"
Write-Host "[DEBUG] rowCount raw: '$rowCount'"
